...
$$\Delta f(x) \approx \diff{f(x)}{x}\cdot \Delta x$$Let $i$, $j$ be the indices of any pair of peaks such that $i < j$, and also let $\gamma_{i, j} = \dfrac{A_i}{A_{j}}$, the following table summarizes the uncertainties involved in the calculation of the damping ratio from a pair of peaks indexed with $(i, j)$.
